...
public class VisualArrayLists implements ActionListener {
	...
	public void addThingsToList() {
		...
		list.add("Wake up at 8");
		list.add("Eat Breakfast");
		list.add("Go play with friends");
		list.add("Eat lunch");
		list.add("Play video games");
		// 2. Set one item in our list to be something different
		list.set(4, "Go to soccer practice");
		...
		if (list.contains("Eat lunch")) {
			System.out.println("Eat lunch is on the list.");
		}
		else {
			System.out.println("Eat lunch is not on the list");
		}
